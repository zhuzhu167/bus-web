import { aUser, fUser, like, userF, findL, myF } from "@/api/user";
...
// 是否登录
export const CheckLogin = ({ commit }, data) => {
  commit("LOGIN", true);
...
// 反馈
export const FeedBack = ({ commit }, data) => {
  userF(data).then((res) => {
    if (res.data.status === 200) {
      wx.navigateBack({
        url: "/pages/user/main",
...
// 我的反馈
export const GetMyFeedback = ({ commit }, data) => {
  myF(data).then((res) => {
    if (res.data.status === 200) {
...
// 获取闹钟列表
export const GetClockList = ({ commit }, data) => {
...
// 设置闹钟
export const SetClock = ({ commit }, data) => {
  wx.navigateBack({
    delta: 3,
    success() {
      $Toast({
...
// 查找失物
export const GetLost = ({ commit }, data) => {
  findL().then((res) => {
    if (res.data.status === 200) {
